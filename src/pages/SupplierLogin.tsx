...
import { Building2, Lock, Phone, ArrowLeft, AlertCircle } from 'lucide-react';
...
export const SupplierLogin: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/supplier-staff/login', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-slate-400">
          Sign in to view incoming trucks queue & confirm loading checkpoints.
        </p>
      </div>
        ...
        <div className="bg-slate-800 py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-slate-700">
          ...
          <form className="space-y-6" onSubmit={handleLogin}>
            ...
            <div>
              ...
              >
                {loading ? 'Signing in...' : 'Sign In as Supplier Staff'}
              </button>
            </div>
          </form>
