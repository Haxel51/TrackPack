  ...
  return (
    <div className={`relative inline-block text-left ${className}`} ref={dropdownRef}>
      <button
        ...
        className="bg-white/10 hover:bg-white/20 text-white border border-white/20 text-xs font-bold px-2 sm:px-2.5 py-1.5 sm:py-2 rounded-xl flex items-center gap-1 sm:gap-1.5 transition-colors cursor-pointer shadow-xs shrink-0 whitespace-nowrap"
        aria-expanded={isOpen}
        aria-haspopup="true"
        title="Change language"
      >
        <Globe className="w-3.5 h-3.5 text-[#F7941D] shrink-0" />
        <span className="text-xs">{currentLanguage.flag}</span>
        <span className="hidden md:inline text-xs truncate max-w-[70px]">{currentLanguage.name}</span>
        <ChevronDown className={`w-3 h-3 text-slate-300 transition-transform duration-200 shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
