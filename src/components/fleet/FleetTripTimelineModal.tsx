    ...
    <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 z-50 overflow-hidden animate-in fade-in duration-200">
      <div className="bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl max-w-xl w-full h-[94vh] sm:h-auto sm:max-h-[92vh] flex flex-col overflow-hidden border border-slate-100">
        ...
        <div className="p-4 sm:p-6 space-y-5 flex-1 overflow-y-auto overscroll-contain pb-24 sm:pb-8">
          ...
          {/* DRIVER INTERFERENCE WARNING BANNER */}
          {trip.last_interference_alert && (
            <div className="bg-gradient-to-r from-rose-950/90 via-slate-900 to-rose-950/80 border-2 border-rose-500 rounded-2xl p-4 text-white text-xs shadow-xl flex flex-col sm:flex-row items-start justify-between gap-3 animate-pulse">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="bg-rose-500 text-white text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full">
                      ⚠️ Driver Interference Alert
                    </span>
                    <span className="text-[10px] text-slate-300 font-mono">
                      {new Date(trip.last_interference_alert.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                    </span>
                  </div>
                  <p className="font-extrabold text-sm text-white leading-snug">
                    {trip.last_interference_alert.message}
                  </p>
                  <p className="text-[11px] text-slate-300 font-medium">
                    Push notification and in-app alerts dispatched to CEO, Manager, and Staff.
                  </p>
                </div>
              </div>

              {trip.last_interference_alert.whatsapp_url && (
                <a
                  href={trip.last_interference_alert.whatsapp_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="bg-[#25D366] hover:bg-[#20bd5a] text-white px-3.5 py-2 rounded-xl font-bold text-xs flex items-center gap-1.5 shrink-0 transition-colors shadow-lg self-end sm:self-center"
                >
              ...
              <div className="grid grid-cols-3 gap-2 pt-1">
                {!isCompleted ? (
                  <button
                    ...
                    className="bg-[#4285F4] hover:bg-[#3367D6] active:scale-95 text-white font-black text-xs py-2.5 px-2 rounded-xl flex items-center justify-center gap-1 shadow-md transition-all cursor-pointer min-h-[42px]"
